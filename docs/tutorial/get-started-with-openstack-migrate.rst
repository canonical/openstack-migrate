...
Getting started with Openstack Migrate
======================================
...
``openstack-migrate`` options.

Let's define a minimal configuration file:

.. code-block:: bash

  export OPENSTACK_MIGRATE_CONFIG=~/migrate-config.yaml
...
The cloud names must match the ones specified in the ``openstack-migrate``
configuration file.

By using the ``OPENSTACK_MIGRATE_CONFIG`` environment variable, we no longer
have to specify it every time we run a ``openstack-migrate`` command.
...
``openstack-migrate`` can automatically identify and migrate resource dependencies.
...
``openstack-migrate cleanup-source`` command, which we are going to showcase
:ref:`a bit later<cleanup_source_command>`.

Batch migrations
----------------

``openstack-migrate`` can also move a group of resources that match a given
filter.
...
.. code-block:: none
  ...
  openstack-migrate cleanup-source --resource-type=image
