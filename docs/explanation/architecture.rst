...
``openstack-migrate`` is a tool written in Python that leverages the
...
``openstack-migrate`` transfers user data such as the volume payloads, images,
...
the data transfer. ``openstack-migrate`` will automatically define access rules
...
of ``openstack-migrate`` by calling the ``register-external`` command. This is
...
``openstack-migrate`` will ignore existing resources that have the same name.

If a resource has been recreated without the use of ``openstack-migrate``, use
...
projects of a given domain), ``openstack-migrate`` will skip already existing
