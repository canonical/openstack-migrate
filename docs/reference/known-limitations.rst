...
This document describes the known limitations of the ``openstack-migrate`` project.

Multi-tenant limitations
------------------------

``openstack-migrate`` currently identifies migrated resources based on UUIDs.
...
moment. This would require extending ``openstack-migrate`` to receive the owner
...
However, users can call ``openstack-migrate list`` to obtain the source and
...
At the same time, ``openstack-migrate`` will pass the ``Force`` flag when uploading
...
``openstack-migrate`` relies on Glance uploads to transfer instance and volume
...
At the moment, ``openstack-migrate`` can only access NFS share exports. Other
...
``openstack-migrate`` is expected to work with any Openstack release starting
with Openstack Zed.
