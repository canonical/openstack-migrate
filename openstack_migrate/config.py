...
class OpenstackMigrateConfig(BaseModel):
    """openstack-migrate coniguration."""
    ...
    database_file: Path = Path(
        os.path.expandvars("$HOME/.local/share/openstack-migrate/sqlite.db")
    )
    ...
    temporary_migration_dir: Path = Path(
        os.path.expandvars("$HOME/.local/share/openstack-migrate/migration_dir")
    )
    ...
    #
    # openstack-migrate will transparently assign the member role to the user
...
_CONFIG: OpenstackMigrateConfig | None = None


def get_config() -> OpenstackMigrateConfig:
    """Retrieve the global config object."""
    global _CONFIG
    if not _CONFIG:
        _CONFIG = OpenstackMigrateConfig()
    return _CONFIG
