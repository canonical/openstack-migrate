...
from openstack_migrate.tests.integration.handlers.barbican import (
    utils as barbican_test_utils,
)
from openstack_migrate.utils import barbican_utils
...
def test_migrate_secret_container_batch(
    ...
    for idx in range(container_count):
        secret_1 = barbican_test_utils.create_test_secret(
            request, test_requester_source_session, binary=True
        )
        secret_2 = barbican_test_utils.create_test_secret(
            request, test_requester_source_session
        )
        container = barbican_test_utils.create_test_secret_container(
            request, test_requester_source_session, [secret_1, secret_2]
        )
    ...
    for source_container in source_containers:
        dest_container = test_requester_destination_session.key_manager.find_container(
            source_container.name
        )
        assert dest_container, "couldn't find migrated resource"
        request.addfinalizer(
            lambda: test_requester_destination_session.key_manager.delete_container(
        ...
        assert not test_requester_source_session.key_manager.find_container(
            source_container.name
    ...
    for source_secret in source_secrets:
        ...
        dest_secret = test_requester_destination_session.key_manager.get_secret(
            barbican_utils.parse_barbican_url(dest_secret_id)
        )
        assert dest_secret, "couldn't find migrated resource"
        request.addfinalizer(
            lambda: test_requester_destination_session.key_manager.delete_secret(
        ...
        source_secret_names = [
            secret.name
            for secret in test_requester_source_session.key_manager.secrets()
        ]
