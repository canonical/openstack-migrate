...
def test_migrate_secret_batch(
    ...
    # The Barbican handlers do not support multi-tenancy, as such we're creating
    # the resources using the migration requester credentials.
    container_count = 3
    source_secrets = []
    for idx in range(container_count):
        secret = barbican_test_utils.create_test_secret(
