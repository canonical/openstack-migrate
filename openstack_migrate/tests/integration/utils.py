...
from openstack_migrate import constants
...
def call_migrate(config_path: Path, command: list[str]):
    command = ["openstack-migrate", "--config", str(config_path)] + command
    LOG.info("Calling openstack-migrate: %s", command)
    subprocess.check_call(command, text=True)


def check_migrate(config_path: Path, command: list[str]) -> str:
    """Run the openstack-migrate command and capture the output."""
    command = ["openstack-migrate", "--config", str(config_path)] + command
    LOG.info("Calling openstack-migrate: %s", command)
