...
from openstack_migrate.tests.integration import config as integration_test_config
...
def base_config_path() -> Path:
    ...
    # configs.
    config_path = os.getenv("OPENSTACK_MIGRATE_CONFIG")
    assert config_path, "OPENSTACK_MIGRATE_CONFIG env variable missing."
    return Path(config_path)


@pytest.fixture(scope="session", autouse=True)
def base_config(base_config_path) -> integration_test_config.OpenstackMigrateTestConfig:
    conf = integration_test_config.OpenstackMigrateTestConfig()
...
def _create_test_project(session, name) -> sdk_project.Project:
    return session.identity.create_project(
        name=name,
        description="openstack-migrate test project",
...
def _create_test_user(session, project, user_name, roles) -> sdk_user.User:
    user = session.identity.create_user(
        ...
        description="openstack-migrate test user",
...
def test_config_path(tmpdir_factory) -> Path:
    """The openstack-migrate config used for this class of tests."""
    return Path(
        tmpdir_factory.mktemp("openstack_migrate").join("openstack-migrate-config.yaml")
    )
...
def test_owner_cloud_config_path(tmpdir_factory) -> Path:
    ...
    return Path(tmpdir_factory.mktemp("openstack_migrate").join("clouds-owner.yaml"))
...
def test_database_path(tmpdir_factory) -> Path:
    """The Openstack clouds.yaml used for this class of tests."""
    return Path(tmpdir_factory.mktemp("openstack_migrate").join("sqlite.db"))


def _prepare_cloud_config(
    base_config: config.OpenstackMigrateConfig,
    project_name: str,
    user_name: str,
    output_path: Path,
):
...
) -> config.OpenstackMigrateConfig:
    LOG.info("Preparing openstack-migrate config: %s", test_config_path)
    conf = config.OpenstackMigrateConfig(**base_config.model_dump())
